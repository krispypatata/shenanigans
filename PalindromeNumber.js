/*
Given an integer x, return true if x is a palindrome, and false otherwise.

Example 1:
Input: x = 121
Output: true
Explanation: 121 reads as 121 from left to right and from right to left.

Example 2:
Input: x = -121
Output: false
Explanation: From left to right, it reads -121. From right to left, it becomes 121-. Therefore it is not a palindrome.

Example 3:
Input: x = 10
Output: false
Explanation: Reads 01 from right to left. Therefore it is not a palindrome.
*/

/**
 * @param {number} x
 * @return {boolean}
 */

// function that reverses a string
var reverseString = function(xString) {
    // transforming the string into an array
    let splitX = xString.split("");

    // reversing the array
    reverseSplitX = splitX.reverse();

    // converting the array back to a string
    reversedX = reverseSplitX.join("");

    // returning the result
    return reversedX;
}
var isPalindrome = function(x) {
    // convert the integer into an array
    let xString = x.toString();

    // comparison
    return xString == reverseString(xString);
};

console.log("Is " + 121 + " a Palindrome? " + isPalindrome(121));
console.log("Is " + 21 + " a Palindrome? " + isPalindrome(21));